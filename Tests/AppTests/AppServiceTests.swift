...
struct AppServiceTests {
    ...
    func create_enrichesAndPersists() async throws {
        let lookup = LookupResultApp(trackId: 42, bundleId: "com.azri", trackName: "Azri", artworkUrl100: "https://icon", primaryGenreId: 6017)
        ...
        #expect(result.primaryGenreId == 6017)
    ...
    func list_returnsRepository() async throws {
        ...
        let lookup = LookupResultApp(trackId: 1, bundleId: "a", trackName: "A", artworkUrl100: nil, primaryGenreId: nil)
        let service = AppService(repository: repo, lookupClient: StubLookupClient(canned: lookup))
