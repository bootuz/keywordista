...
struct AuthIntegrationTests {
    ...
    struct SetupTests {
        ...
        // ── M3.21: SETUP_TOKEN defense-in-depth ───────────────────

        @Test("M3.21: when no setupToken is configured, /setup ignores any header (pre-M3.21 behavior)")
        func tokenBackcompatNoToken() async throws {
            // Default AuthTestApp.make() passes setupToken=nil — the
            // /setup route is wide open. Existing M1.12 tests rely on
            // this; pinning here protects that backcompat surface.
            let app = try await AuthTestApp.make()
            defer { Task { try? await app.asyncShutdown() } }
            ...
            try await app.test(
                .POST, "/api/v1/auth/setup",
                headers: HTTPHeaders([
                    // A bogus header should be ignored when no token
                    // is configured — we don't want a confused
                    // attacker to be able to *block* setup either.
                    ("X-Keywordista-Setup-Token", "garbage-shouldnt-matter"),
                ]),
                beforeRequest: { req in
                    try req.content.encode([
                        "email": "founder@studio.local",
        ...
        @Test("M3.21: when setupToken is configured, /setup without the header returns 401")
        func tokenRequiredButMissing() async throws {
            let app = try await AuthTestApp.make(
                setupToken: "long-enough-test-token-please-yes"
            )
            defer { Task { try? await app.asyncShutdown() } }
            ...
            // Token check must run BEFORE user creation — no row
            // should have been written.
            let count = try await User.query(on: app.db).count()
            #expect(count == 0)
        }

        @Test("M3.21: wrong token returns 401, no user created")
        func tokenWrongValue() async throws {
            let app = try await AuthTestApp.make(
                setupToken: "long-enough-test-token-please-yes"
            )
            defer { Task { try? await app.asyncShutdown() } }
            ...
            try await app.test(
                .POST, "/api/v1/auth/setup",
                headers: HTTPHeaders([
                    ("X-Keywordista-Setup-Token", "different-but-also-long-enough"),
                ]),
                beforeRequest: { req in
                    try req.content.encode([
                        "email": "founder@studio.local",
        ...
        @Test("M3.21: correct token grants /setup → 201 + admin user")
        func tokenHappyPath() async throws {
            let token = "long-enough-test-token-please-yes"
            let app = try await AuthTestApp.make(setupToken: token)
            defer { Task { try? await app.asyncShutdown() } }

            try await app.test(
                .POST, "/api/v1/auth/setup",
                headers: HTTPHeaders([
                    ("X-Keywordista-Setup-Token", token),
                ]),
                beforeRequest: { req in
                    try req.content.encode([
                        "email": "founder@studio.local",
        ...
        @Test("M3.21: token check runs BEFORE the 410-already-set-up check")
        func tokenCheckedBeforeAlreadySetUp() async throws {
            // Subtle but important: after first-run completes, hitting
            // /setup with NO token should still return 401 — not 410.
            // Reason: 410 leaks "you're at a real Keywordista server,
            // it's just already set up" to an unauthenticated scanner.
            // 401 says "you're not authorized to know" — strictly less
            // information disclosure.
            let token = "long-enough-test-token-please-yes"
            let app = try await AuthTestApp.make(setupToken: token)
            defer { Task { try? await app.asyncShutdown() } }

            // First-run: seed an admin so the table is non-empty.
            try await app.test(
                .POST, "/api/v1/auth/setup",
                headers: HTTPHeaders([
                    ("X-Keywordista-Setup-Token", token),
            ...
            // Second attempt, NO header → 401 (not 410).
            try await app.test(
                .POST, "/api/v1/auth/setup",
                beforeRequest: { req in
                    try req.content.encode([
                        "email": "intruder@studio.local",
                        "password": "another-strong-pw",
                    ])
                },
                afterResponse: { res async in
                    #expect(res.status == .unauthorized,
                           "no-token check must precede 410 to avoid leaking 'real server, already set up'")
                }
            )
        }

        @Test("M3.21: /auth/state surfaces setupTokenRequired = false when no token configured")
        func stateSurfacesNoTokenRequired() async throws {
            let app = try await AuthTestApp.make()
            defer { Task { try? await app.asyncShutdown() } }
            ...
            try await app.test(
                ...
                afterResponse: { res async in
                    ...
                    #expect(body?.setupTokenRequired == false)
                }
            )
        }

        @Test("M3.21: /auth/state surfaces setupTokenRequired = true when configured")
        func stateSurfacesTokenRequired() async throws {
            let app = try await AuthTestApp.make(
                setupToken: "long-enough-test-token-please-yes"
            )
            defer { Task { try? await app.asyncShutdown() } }
            ...
            try await app.test(
                ...
                afterResponse: { res async in
                    #expect(res.status == .ok)
                    let body = try? res.content.decode(AuthStateResponse.self)
                    #expect(body?.setupTokenRequired == true)
                }
            )
        }
    }
