...
<div class="relative z-20 space-y-3 border-b border-zinc-800 bg-zinc-950/60 px-6 py-3">
  ...
  <div class="flex flex-wrap items-center gap-3">
    <input
      ...
      class="w-64 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-1 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-zinc-600 focus:outline-none"
    />
    <select
      bind:value={$groupBy}
      class="rounded-md border border-zinc-800 bg-zinc-900 px-2 py-1 text-sm text-zinc-100"
    >
    ...
    <!-- Rank bucket pills -->
    <div class="flex items-center gap-1 rounded-md border border-zinc-800 bg-zinc-900 p-0.5">
      {#each rankBuckets as bucket}
        <button
          type="button"
          onclick={() => setRankBucket(bucket.id)}
          class="rounded px-2 py-0.5 text-xs transition"
          class:bg-zinc-700={$filters.rankBucket === bucket.id}
          class:text-zinc-100={$filters.rankBucket === bucket.id}
          class:text-zinc-400={$filters.rankBucket !== bucket.id}
        >
          {bucket.label}
        </button>
      {/each}
    ...
    {#if isDirty}
      <button
        ...
        class="ml-auto text-xs text-zinc-500 underline-offset-2 hover:text-zinc-300 hover:underline"
      >
        Clear filters
      </button>
    {/if}
  ...
  <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
    ...
    <label class="flex items-center gap-1 text-xs text-zinc-400">
      ...
      <select
        ...
        class="rounded border border-zinc-800 bg-zinc-900 px-1 py-0.5 text-zinc-100"
      >
