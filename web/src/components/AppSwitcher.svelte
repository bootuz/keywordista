...
<div class="relative" bind:this={rootEl}>
  <button
    type="button"
    onclick={() => (open = !open)}
    class="inline-flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-900 px-2.5 py-1 text-sm text-zinc-100 hover:bg-zinc-800"
  ...
  >
    {#if currentApp?.iconURL}
      <img
        ...
        width="18"
        height="18"
        class="rounded-[4px] ring-1 ring-zinc-800"
      />
    {/if}
  ...
  {#if open}
    ...
    >
          ...
          >
            {#if app.iconURL}
              <img
                ...
                width="20"
                height="20"
                class="rounded-[5px] ring-1 ring-zinc-800"
              />
            {:else}
              <span class="h-5 w-5 rounded-[5px] bg-zinc-800"></span>
            {/if}
            <span class="flex-1 truncate text-zinc-200">{app.name}</span>
            {#if active}
      ...
      <button
        type="button"
        onclick={addAndClose}
        class="flex w-full items-center gap-2 border-t border-zinc-800 bg-zinc-950 px-3 py-2 text-left text-sm text-amber-400 hover:bg-zinc-800"
      >
